/* RCS compile-time configuration for DOS and OS/2 */
	...
	/* $Id: conf.h,v 1.12 1995/06/05 08:36:40 eggert Exp $ */
...
#include <crtrsxnt.h>
#endif
#if defined(__WIN32__) && defined(RCSDLL) && defined(RCSMAIN)
#include <rcslib.h>
#endif
...
/* emulate symlinks via ./RCS file */
#define pseudo_symlinks 1

/* fix working file mode */
#define bad_workmode 1

/* Comment out the typedefs below if the types are already declared.  */
/* Fix any uncommented typedefs that are wrong.  */
#if !defined(_MODE_T) && !defined(_MODE_T_) && !defined(__CYGWIN__)
typedef int mode_t;
#endif
/* typedef long off_t; */
/* typedef int pid_t; */
...
#if !defined(__EMX__) && !defined(__MINGW32__) && !defined(__IBMC__)
#define const
#endif
#if defined(__WIN32__) && defined(RCSDLL)
/* gcc under Win32 does not support pointer constants
   exported from a DLL being marked as const */
#define const
#endif
#ifdef __WATCOMC__
#define volatile
...
#	define FOPEN_WB "wb"
#	define FOPEN_W_WORK ((Expand==BINARY_EXPAND) || getenv("RCS_LF_ONLY") ? "wb" : "w")
...
#define OPEN_CREAT_READONLY (S_IRUSR|S_IWUSR) /* lock file mode */
#else
...
#else /* __MSDOS__ */
#define OPEN_O_WRONLY O_RDWR /* Lock files are unreliable under DOS!  */
#endif
...
#define has_attribute_noreturn ((2 == __GNUC__ && 6 <= __GNUC_MINOR__) || 3 <= __GNUC__) /* Does __attribute__((noreturn)) work?  */
...
#define has_si_errno 0 /* Does siginfo_t have si_errno?  */
...
#define has_attribute_format_printf ((2 == __GNUC__ && 6 <= __GNUC_MINOR__) || 3 <= __GNUC__) /* Does __attribute__((format(printf,N,N+1))) work?  */
...
#	else
#           if defined(__EMX__) && (defined(__OS2__) || defined(__WIN32__))
...
/* this is Win32 specific, and cannot be solved outside
   the source code, unfortunately */

#if defined(__WIN32__) && defined(RCSIMP)
#undef RCSIMP
#ifdef _MSC_VER
#define RCSIMP __declspec(dllimport)
#endif
#ifdef __IBMC__
#define RCSIMP _Import
#endif
#endif
...
#define utime(f, b) utime((char *) (f), (b))

#define chmod(f, m) ibmc_chmod(f, m)
#define rename(f, t) ibmc_rename(f, t)
#define unlink(f) ibmc_unlink(f)
#define dup2(f, t) ibmc_dup2(f, t)
